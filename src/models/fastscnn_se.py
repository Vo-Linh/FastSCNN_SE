...
__all__ = ['SE_FastSCNN', 'get_fast_scnn']
...
class SEModule(nn.Module):
    ...
    def __init__(self, channels, reduction=4):
        ...
        self.conv = ConvBlock(In_Channels=channels // reduction,
                              Out_Channels=channels, Kernel_Size=1, Stride=1)

    def forward(self, x):
        module_input = x
        avg_out = self.avg_pool(x)
        fc_out = self.fc1(avg_out)
        sigmoid_out = self.sigmoid(fc_out)
        fc_out = self.relu(fc_out)
        multi_out = fc_out * sigmoid_out
        conv_out = self.conv(multi_out)

        x = self.relu(conv_out)
        return module_input * x


class SE_FastSCNN(nn.Module):
    def __init__(self, num_classes, aux=False, **kwargs):
        super(SE_FastSCNN, self).__init__()
        self.aux = aux
...
class LearningToDownsample(nn.Module):
    ...
    def __init__(self, dw_channels1=32, dw_channels2=48, out_channels=64, **kwargs):
        ...
        self.conv = _ConvBNReLU(5, dw_channels1, 3, 2)
...
class FeatureFusionModule(nn.Module):
    ...
    def forward(self, higher_res_feature, lower_res_feature):
        ...
        lower_res_feature = self.se_module(lower_res_feature)
